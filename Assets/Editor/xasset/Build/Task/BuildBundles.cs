...
namespace xasset.editor
{
    public class BuildBundles : BuildTaskJob
    {
        ...
        {
            BuildRules buildRule = RuleTools.GetBuildRule();
            AssetBundleBuild[] _builds = buildRule.GetBuilds();
            foreach(AssetBundleBuild bundleBuild in _builds)
            {
                ManifestBundle _manifBundle = new ManifestBundle();
                _manifBundle.name = bundleBuild.assetBundleName.Replace("/","-");
                _manifBundle.assets = new List<string>(bundleBuild.assetNames);

                bundles.Add(_manifBundle);
            }
